                ...
                return buildProduct({
                    ...
                    scale: {
                        ...
                        gradient: function(v, a){
                            var segments = [
                                [0,       window.haxSettings.MAP_COLOR_0],
                                [15,      window.haxSettings.MAP_COLOR_1],
                                [30,      window.haxSettings.MAP_COLOR_2],
                                [60,      window.haxSettings.MAP_COLOR_3],
                                [100,     window.haxSettings.MAP_COLOR_4]
                            ];

                            for(var i = 0; i<segments.length; i++){
                                if(v < segments[i][0]){
                                    var up = segments[Math.min(i, segments.length-1)][1];
                                    var low = segments[Math.max(i-1, 0)][1];

                                    var upBound = segments[Math.min(i, segments.length-1)][0];
                                    var lowBound = segments[Math.max(i-1, 0)][0];
                                    var d = upBound - lowBound;

                                    var r = (v - lowBound)/d;

                                    var interp = µ.colorInterpolator(low, up);
                        ...
                        /*
                        function(v, a) {
                            var r = Math.min(v, 100) / 100;
                            return µ.extendedSinebowColor(r, window.haxSettings.MAP_ALPHA*255/100);
                        }*/
                    },
