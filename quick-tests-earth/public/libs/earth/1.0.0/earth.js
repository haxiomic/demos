...
(function() {
    ...
    var MIN_SLEEP_TIME = 0;                  // amount of time a task waits before resuming (millis)
    ...
    function animate(globe, field, grids) {
        ...
        for (var i = 0; i < particleCount; i++) {
            particles.push(field.randomize({age: _.random(0, haxSettings.MAX_PARTICLE_AGE)}));
        }

        function evolve() {
            buckets.forEach(function(bucket) { bucket.length = 0; });
            particles.forEach(function(particle) {
                if (particle.age > haxSettings.MAX_PARTICLE_AGE) {
                ...
                var m = v[2];
                if (m === null) {
                    particle.age = haxSettings.MAX_PARTICLE_AGE;  // particle has escaped the grid, never to return...
                }
                else {
                    var xt = x + v[0]*haxSettings.VELOCITY_MULTIPLIER;
                    var yt = y + v[1]*haxSettings.VELOCITY_MULTIPLIER;
        ...
        function draw() {
            ...
            buckets.forEach(function(bucket, i) {
                if (bucket.length > 0) {
                    ...
                    //@! color hack
                    //extract level
                    var level = parseInt(colorStyles[i].match(/\d+/)[0])/255;

                    var col = 'rgb(' + 
                            Math.round(haxSettings.COLOR.r*level) + ',' +
                            Math.round(haxSettings.COLOR.g*level) + ',' +
                            Math.round(haxSettings.COLOR.b*level) +
                            ')';
